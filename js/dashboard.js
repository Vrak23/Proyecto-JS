...
$('.sidebar-nav li').on('click', function (e) {
    ...
    // Obtener texto del menú clickeado
    let menuTexto = $(this).text().trim();
    ...
    if (menuTexto === 'Dashboard') {
        $('#tituloPagina').text('Dashboard');
        $('#moduloDashboard').fadeIn();
        cargarDatosDashboard();

    } else if (menuTexto === 'Estudiantes') {
        $('#tituloPagina').text('Mantenimiento de Alumnos');
        ...
        $('#moduloDashboard').fadeIn();
        alert('Módulo en desarrollo. Esta sección estará disponible pronto.');
    }
});
...
// — Gráfico Dona: Alumnos por Género —
...
// — Gráfico Barras: Vacantes por Nivel Educativo —
